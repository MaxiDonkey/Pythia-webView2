...
uses
  ...
  FMX.Forms, FMX.Controls, FMX.Types, FMX.Graphics, FMX.Dialogs, FMX.Styles,
...
type
  EFMXPythiaException = class(Exception);

  TFMXAlphaBlend = record
    class procedure SetWindowAlpha(const Form: TForm;
      AAlpha: Byte;
      AEnableLayered: Boolean = False); static;
    class procedure HideWindow(const Form: TForm); static;
    class procedure ShowWindow(const Form: TForm); static;
  end;

  TFMXOpenDialog = class(TInterfacedObject, IOpenDialog)
  private
  ...
  TFMXPythiaAbstract = class(TComponent)
  protected
  ...
  TFMXPythiaPath = class(TFMXPythiaAbstract)
  strict private
    const
      MEDIA_FOLDER = 'media';
  ...
  TFMXPythiaCore = class(TFMXPythiaPath)
  strict private
    const
      TIMER_INTERVAL = 300;
  ...
  protected
    ...
    procedure DoLayoutResize(Sender: TObject);
    procedure DoOnTimer(Sender: TObject);
  public
    constructor Create(AOwner: TComponent); override;
    procedure AttachHost(AForm: TForm);
    function Update: Boolean;
    property Browser: TWVFMXBrowser read FBrowser;
  ...
  TFMXPythiaThemeManager = class(TFMXPythiaJSTemplatesManager)
  strict private
    const
      DARK_BACKGROUND_COLOR = $00272727;
      LIGHT_BACKGROUND_COLOR = TColor(TAlphaColorRec.White);
  private
    FTheme: string;
  protected
  ...
  TFMXPythiaScrollManager = class(TFMXPythiaThemeManager)
  strict private
    const
      CLEAR_RATE = 1;
      BIAS_DEFAULT = 300;
  ...
  TFMXPythiaLanguageManager = class(TFMXPythiaScrollManager)
  private
  ...
  TFMXPythiaAppSettings = class(TFMXPythiaLanguageManager)
  protected
    procedure SettingsPanelSaveAppSettings; virtual;
    procedure SaveDefaultValues;
  end;

  TFMXPythiaDialogManager = class(TFMXPythiaAppSettings)
  private
  ...
  TFMXPythiaRunProcessManager = class(TFMXPythiaDialogManager)
  private
  ...
  TFMXPythiaChatContentManager = class(TFMXPythiaRunProcessManager)
  private
  ...
  TFMXPythiaCustomJSTemplate = class(TFMXPythiaChatContentManager)
  private
  ...
  TFMXPythiaBridgeManager = class(TFMXPythiaCustomJSTemplate)
  strict private
    const
      MSG_READY = '"ready"';
  ...
  TFMXPythiaAPIKeyManager = class(TFMXPythiaInputValue)
  private
  ...
  TFMXPythiaLockServices = class(TFMXPythiaAPIKeyManager)
  private
    FLocked: Boolean;
    FEscape: Boolean;
  ...
  TFMXPythiaCustomPanels = class(TFMXPythiaLockServices)
  private
  ...
  TFMXPythiaSettingsPanel = class(TFMXPythiaCustomPanels)
  private
  ...
  TFMXPythiaModelsSelector = class(TFMXPythiaSettingsPanel)
  private
  ...
  TFMXPythiaChatSessionManager = class(TFMXPythiaModelsSelector)
  strict private
    const
      PAGE_SIZE_VALUE = 25;
  private
  ...
  TFMXPythiaCardSelector = class(TFMXPythiaChatSessionManager)
  private
  ...
  TFMXPythiaInputBubble = class(TFMXPythiaCardSelector)
  private
  ...
  TFMXPythiaReasoningComponent = class(TFMXPythiaInputBubble)
  private
  ...
  TFMXPythiaCommandLine = class(TFMXPythiaReasoningComponent)
  private
  ...
  TInterfacedFMXPythia = class(TFMXPythiaKnowledgeIndexingManager, IPythiaBrowser)
  strict private
    const
      CLEARANCE = 400;
  ...
  TFMXPythia = class(TInterfacedFMXPythia)
  protected
...
(*
    Developer Note   FMX.Browser.Chat

    This unit hosts the FMX / WebView2 browser stack used by the chat UI.

    -------------------------------------------------------------------------
    Platform scope
    -------------------------------------------------------------------------

    This is a Windows-only FMX unit. Although FireMonkey is a cross-platform
    framework, the unit depends on Microsoft WebView2 through TWVFMXPythia
    and IWVFMXHost, both of which have no Android, iOS, macOS or Linux
    counterpart. Targeting a mobile or non-Windows platform would require a
    completely different rendering engine and a full re-implementation of
    the TFMXPythia stack against that engine. Do not assume any FMX-level
    portability here.
    ...
    The IPythiaBrowser contract (Browser.Chat.Interfaces) aggregates a large
    ...
    class (TInterfacedFMXPythia) without turning it into a mega-class of
    ...
    Bootstrap layer
        ...
        TFMXPythiaCore:
          Owns the WebView2 plumbing (TWVFMXPythia, IWVFMXHost, the host
          TLayout), the startup timer, the virtual host mapping and the
          initial navigation. EnsureBrowserInitialized is the single path
          through which the browser comes up; both DoOnTimer and Update
          delegate to it.

    Data / configuration layers
        TFMXPythiaCapabilitiesManager:
    ...
    Fa ade
        TInterfacedFMXPythia:
          Concrete implementation of IPythiaBrowser. Acts as the fa ade used by
          ...
          Its breadth is intentional and dictated by IPythiaBrowser. All the
    ...
    Concrete publishable class
        TFMXPythia:
          ...
          properties. Registered in Register.
    ...
    2. TFMXPythiaCore.AttachHost binds the IWVFMXHost to the host form
       and the TLayout container.
    ...
    8. Event handlers (Browser.Chat.EventHandlers) call IPythiaBrowser methods
       implemented by TInterfacedFMXPythia to mutate the UI.
    ...
    -------------------------------------------------------------------------
      ...
      FEventManager is created early (in TFMXPythiaBridgeManager) but
      receives its runtime dependencies only after TInterfacedFMXPythia
      construction.
      PromptCount is the alignment key between:
        - DOM rendering (browser side)
        - persisted chat turns (backend)
      Every abstract primitive declared in TFMXPythiaAbstract must have
      ...
      prevents TFMXPythia from being instantiated.

    -------------------------------------------------------------------------
    Architectural constraints (intentional design choices)
    -------------------------------------------------------------------------

      TFMXPythiaBridgeManager groups every WebView2 runtime concern by
      design. Splitting navigation, messaging and injection would break
      lifecycle coherence.
      TInterfacedFMXPythia is a wide fa ade by necessity, since IPythiaBrowser
      aggregates rendering, session, input and UI control
      responsibilities.
      TFMXPythiaCore centralizes initialization because WebView2 startup,
    ...
    -------------------------------------------------------------------------
      ...
      Do not add rendering logic to TFMXPythiaBridgeManager.
      Do not move WebView2 lifecycle logic into TInterfacedFMXPythia.
      Do not extend TFMXPythiaCore beyond infrastructure / bootstrap.
      Do not bypass IPythiaBrowser when interacting with the UI from event
      handlers.
      ...
      Keep this unit architecturally aligned with VCL.Browser.Chat. Drift
...
function TInterfacedFMXPythia.ClearBrowserDisplay: Boolean;
begin
  Result := ExecuteScript(CLEAR_TEMPLATE);
end;

procedure TInterfacedFMXPythia.ClearCurrentChatSession;
begin
  if Assigned(FPersistentChat) then
    FPersistentChat.CurrentChat := nil;
end;

procedure TInterfacedFMXPythia.ClearInternalBrowserData;
begin
...
procedure TInterfacedFMXPythia.ClearMediaPlayer;
begin
  StopMedia;
end;

function TInterfacedFMXPythia.Confirmation(
...
constructor TInterfacedFMXPythia.Create(AOwner: TComponent);
begin
...
function TInterfacedFMXPythia.DisplayFooter(const Value: string): Boolean;
begin
  Result := SetChatFooter(Value);
end;

function TInterfacedFMXPythia.DisplayMedia(Kind: TDisplayKind;
...
function TInterfacedFMXPythia.DisplayStream(const AText: string;
  Scroll: Boolean): Boolean;
begin
  Result := DisplayStream(AText, '', Scroll);
end;

function TInterfacedFMXPythia.DisplayStream(const AText, AThink: string;
  Scroll: Boolean): Boolean;
begin
    if not IsBrowserReady then
    Exit(False);
...
function TInterfacedFMXPythia.DisplaySuccess(const Value: string): Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TInterfacedFMXPythia.DisplayWarning(const Value: string): Boolean;
begin
  Result := PostWebMessageAsJson(
...
procedure TInterfacedFMXPythia.EndUpdate;
begin
  {--- Flush the current render batch and allow the browser to repaint. }
  if not ExecuteScript(RENDER_BATCH_END_UPDATE) then
    raise EFMXPythiaException.Create(S_BATCH_END_ERROR);
end;

function TInterfacedFMXPythia.ExecuteTemplate(
  const Template: string;
  const Value: TArray<string>;
  const isPromptSource: Boolean;
...
function TInterfacedFMXPythia.GetSettingsPanelPage: Integer;
begin
  Result := FSettingsPanelPage;
end;

procedure TInterfacedFMXPythia.Hide;
begin
  FWindowParent.Visible := False;
end;

function TInterfacedFMXPythia.InternalDisplaySession: Boolean;
begin
...
function TInterfacedFMXPythia.Prompt(const AText: string): Boolean;
begin
  if not IsBrowserReady then
    Exit(False);
...
function TInterfacedFMXPythia.RenderMedia(
  const Template: string;
  const Value: TArray<string>;
  const isPromptSource: Boolean;
  const Align: string;
...
procedure TInterfacedFMXPythia.UpdateEnabledButtons;
begin
...
function TInterfacedFMXPythia.UpdateFileDrawer: Boolean;
begin
...
procedure TInterfacedFMXPythia.SetSettingsPanelPage(const Value: Integer);
begin
  FSettingsPanelPage := Value;
end;

procedure TInterfacedFMXPythia.SetTheme(const Value: string);
begin
  Theme := Value;
end;

procedure TInterfacedFMXPythia.Show;
begin
  if not FWindowParent.Visible then
    FWindowParent.Visible := True;
end;

procedure TInterfacedFMXPythia.StopAudio;
begin
...
procedure TInterfacedFMXPythia.StopVideo;
begin
  if not ExecuteScript(STOP_VIDEO_TEMPLATE) then
    raise EFMXPythiaException.Create(S_STOP_VIDEO_MEDIA_ERROR);
end;
...
constructor TFMXPythiaCore.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ...
  FWindowParent := TLayout(AOwner);
  FHost := CreateWVFMXHost;
...
procedure TFMXPythiaCore.CreateMappingFolder;
begin
...
procedure TFMXPythiaCore.DoAfterCreated(Sender: TObject);
begin
  if FHost <> nil then
    FHost.ResizeChild;
...
procedure TFMXPythiaCore.DoLayoutResize(Sender: TObject);
begin
  if FHost <> nil then
    FHost.ResizeChild;
end;

procedure TFMXPythiaCore.DoOnTimer(Sender: TObject);
begin
  FTimer.Enabled := False;
...
function TFMXPythiaCore.EnsureBrowserInitialized: Boolean;
begin
  Result := False;
  ...
  var ParentHandle := FHost.GetBrowserParentHandle;
  Result := FBrowser.CreateBrowser(ParentHandle)
end;

function TFMXPythiaCore.GetLocalHost: string;
begin
  Result := LOCAL_HOST;
end;

function TFMXPythiaCore.Update: Boolean;
begin
...
procedure TFMXPythiaBridgeManager.BridgeInitialize;
begin
...
constructor TFMXPythiaBridgeManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ...
  {--- The event manager is created early, then completed later
       when TInterfacedFMXPythia injects the concrete runtime dependencies. }
  FEventManager := TBrowserEventManager.Create;
end;

procedure TFMXPythiaBridgeManager.DoInjectionsWhenReady;
begin
...
procedure TFMXPythiaBridgeManager.DoNavigationCompleted(Sender: TObject;
  const aWebView: ICoreWebView2;
  const aArgs: ICoreWebView2NavigationCompletedEventArgs);
begin
...
procedure TFMXPythiaBridgeManager.DoPermissionRequested(Sender: TObject;
  const aWebView: ICoreWebView2;
  const aArgs: ICoreWebView2PermissionRequestedEventArgs);
var
...
procedure TFMXPythiaBridgeManager.DoWebMessageReceived(Sender: TObject;
  const aWebView: ICoreWebView2;
  const aArgs: ICoreWebView2WebMessageReceivedEventArgs);
var
  pMsg: PWideChar;
...
function TFMXPythiaBridgeManager.ExecuteScript(const Script: string): Boolean;
begin
  if not IsBrowserReady then
    Exit(False);
...
function TFMXPythiaBridgeManager.IsAllowedNavigation(const Url: string): Boolean;
begin
...
function TFMXPythiaBridgeManager.IsBrowserReady: Boolean;
begin
...
function TFMXPythiaBridgeManager.IsJSScriptInjected: Boolean;
begin
...
function TFMXPythiaBridgeManager.PostWebMessageAsJson(const Script: string): Boolean;
begin
...
function TFMXPythiaBridgeManager.PostWebMessageAsJson(
  const Script,
...
function TFMXPythiaCapabilitiesManager.CapabilitiesInitialization: Boolean;
begin
...
constructor TFMXPythiaCapabilitiesManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
function TFMXPythiaCapabilitiesManager.ResetCapabilities: Boolean;
begin
  FCapabilities.Reset;
  Result := UpdateCapabilities;
end;

procedure TFMXPythiaCapabilitiesManager.SaveDefaultCapabilitiesFile;
begin
...
function TFMXPythiaCapabilitiesManager.UpdateCapabilities: Boolean;
begin
...
constructor TFMXPythiaThemeManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ...
  FBrowser.DefaultBackgroundColor := DARK_BACKGROUND_COLOR;
end;

procedure TFMXPythiaThemeManager.SetInternalTheme(const Value: string);
var
...
begin
  ...
  case ThemeSelected of
    light:
      FBrowser.DefaultBackgroundColor := LIGHT_BACKGROUND_COLOR
    else
      FBrowser.DefaultBackgroundColor := DARK_BACKGROUND_COLOR;
  end;
  ...
  {--- Apply the selected theme to both the FMX host and the injected browser UI. }
  if not ExecuteScript(Format(SET_THEME_TEMPLATE, [FTheme, FTheme])) then
    raise EFMXPythiaException.Create(S_INTERNAL_THEME_ERROR);
end;

{ TFMXPythiaDialogManager }

constructor TFMXPythiaDialogManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FOpenDialog := TFMXOpenDialog.Create;
end;

{ TFMXPythiaChatContentManager }

function TFMXPythiaChatContentManager.GetPromptCount: Integer;
begin
  Result := FPromptCount;
end;

procedure TFMXPythiaChatContentManager.SetPromptCount(const Value: Integer);
begin
  FPromptCount := Value;
end;
...
constructor TFMXOpenDialog.Create;
begin
  inherited Create;
end;

function TFMXOpenDialog.Execute(const Filter: string;
  const index: Integer;
...
constructor TFMXPythiaRunProcessManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TFMXPythiaAdapter.SetServiceAdapter(
...
function TFMXPythiaChatSessionManager.ChatSessionAdd(const ID,
  Text: string): Boolean;
begin
  Result := PostWebMessageAsJson(
...
procedure TFMXPythiaChatSessionManager.ChatSessionAutoRename(
  const ID: string;
...
function TFMXPythiaChatSessionManager.ChatSessionDrawerClear: Boolean;
begin
  Result := PostWebMessageAsJson(
    FILE_DRAWER_CLEAR,
    'files-drawer-clear'
...
function TFMXPythiaChatSessionManager.ChatSessionDrawerClose: Boolean;
begin
  Result := PostWebMessageAsJson(
    FILE_DRAWER_CLOSE,
    'files-drawer-close'
...
function TFMXPythiaChatSessionManager.ChatSessionDrawerOpen: Boolean;
begin
  Result := PostWebMessageAsJson(
    FILE_DRAWER_OPEN,
    'files-drawer-open'
...
function TFMXPythiaChatSessionManager.ChatSessionRemove(
...
function TFMXPythiaChatSessionManager.ChatSessionRename(const Id,
...
function TFMXPythiaChatSessionManager.ChatSessionToTop(
  const Id: string): Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TFMXPythiaChatSessionManager.ChatSessionUnselect: Boolean;
begin
  Result := PostWebMessageAsJson(
...
constructor TFMXPythiaChatSessionManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FLastPageId := '';
  ...
  FPersistentChat.LocalFileName := System.IOUtils.TPath.GetFullPath(GetChatSessionsFileName);
...
function TFMXPythiaChatSessionManager.GetOnChatSessionAutoRename: TProc<string, string>;
begin
...
procedure TFMXPythiaChatSessionManager.SessionAutoRename(const Id,
  ATitle: string);
begin
...
procedure TFMXPythiaChatSessionManager.SetOnChatSessionAutoRename(
...
function TFMXPythiaChatSessionManager.SettingsPanelLoadPage: Boolean;
begin
  if not Assigned(FPersistentChat) then
    Exit(False);
...
constructor TFMXPythiaLockServices.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FLocked := False;
  FEscape := False;
...
function TFMXPythiaLockServices.GetEscape: Boolean;
begin
  Result := FEscape;
end;

function TFMXPythiaLockServices.GetLocked: Boolean;
begin
  Result := FLocked;
end;
...
procedure TFMXPythiaLockServices.SetLocked(const Value: Boolean);
const
...
function TFMXPythiaScrollManager.GetHeightAfter(Bias: Integer): Integer;
begin
  {--- Compute the remaining viewport height used as a scroll target offset. }
  if Bias = 0 then
    Result := Trunc(FWindowParent.Height - Trunc(FWindowParent.Height * CLEAR_RATE))
  else
    Result := Trunc(FWindowParent.Height - Bias);

  if Result < 0 then
    Result := 0;
end;

function TFMXPythiaScrollManager.GetScrollButtonsVisible: Boolean;
begin
  Result := FScrollButtonsVisible;
end;

procedure TFMXPythiaScrollManager.ScrollButtonsVisible(Value: Boolean);
begin
  ExecuteScript(
...
procedure TFMXPythiaScrollManager.ScrollToAfterEnd(Smooth: Boolean);
begin
  ScrollToAfterEnd(GetHeightAfter(0), Smooth);
end;

procedure TFMXPythiaScrollManager.ScrollToEnd(Smooth: Boolean);
begin
  if Smooth then
...
procedure TFMXPythiaScrollManager.ScrollToTop(Smooth: Boolean);
begin
  if Smooth then
...
procedure TFMXPythiaScrollManager.SetScrollButtonsVisible(
  const Value: Boolean);
begin
...
procedure TFMXPythiaScrollManager.ScrollToAfterEnd(SizeAfter: Integer;
  Smooth: Boolean);
var
  ScrollCmd: string;
begin
...
constructor TFMXPythia.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);

end;

procedure TFMXPythia.Initialize;
begin
...
class function TFMXPythia.Version: string;
begin
...
function TFMXPythiaSettingsPanel.SettingsPanelForceLanguageSelection(
  const JsonAsString: string): Boolean;
begin
  Result := PostWebMessageAsJson(JsonAsString, 'set-language');
end;

function TFMXPythiaSettingsPanel.SettingsPanelInitializeFullState(const JsonAsString: string): Boolean;
begin
  Result := PostWebMessageAsJson(JsonAsString, 'request-initialization');
end;

function TFMXPythiaSettingsPanel.SettingsPanelRequestCurrentSettingsState: Boolean;
begin
  Result := PostWebMessageAsJson(SETTINGS_PANEL_GET_VALUES, 'request-params-get-values');
end;

function TFMXPythiaSettingsPanel.SettingsPanelUpdateApplicationSettings(
  const JsonAsString: string): Boolean;
begin
  Result := PostWebMessageAsJson(JsonAsString, 'request-params-main-values');
end;

function TFMXPythiaSettingsPanel.SettingsPanelUpdatePropertiesByFullPath(
  const JsonAsString: string): Boolean;
begin
  Result := PostWebMessageAsJson(JsonAsString, 'request-params-update');
end;

function TFMXPythiaSettingsPanel.SettingsPanelGetValues: Boolean;
begin
  Result := ExecuteScript(SETTING_PANEL_GET_VALUES);
end;

function TFMXPythiaSettingsPanel.SettingsPanelHide: Boolean;
begin
  Result := PostWebMessageAsJson(SETTINGS_PANEL_HIDE, 'request-params-hide');
end;

function TFMXPythiaSettingsPanel.SettingsPanelShowPage(const Page: Integer): Boolean;
begin
  Result := PostWebMessageAsJson(
...
constructor TFMXPythiaLanguageManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FOnTranslationsLoaded := nil;
end;

function TFMXPythiaLanguageManager.GetDictionaryFileName(
  const Value: string;
...
function TFMXPythiaLanguageManager.GetLocalLanguage: string;
begin
  Result := FLocalLanguage;
end;

function TFMXPythiaLanguageManager.GetNormalizedFileNames: string;
begin
...
function TFMXPythiaLanguageManager.LoadDictionaryContent(
...
procedure TFMXPythiaLanguageManager.SetLanguage(const Value: string);
begin
  LocalLanguage := Value;
  BubbleInputWelcome(S_WELCOME);
end;

procedure TFMXPythiaLanguageManager.SetLocalLanguage(const Value: string);
var
  FileName: string;
begin
...
{ TFMXPythiaAppSettings }

procedure TFMXPythiaAppSettings.SaveDefaultValues;
begin
  SettingsPanelSaveAppSettings;
end;

procedure TFMXPythiaAppSettings.SettingsPanelSaveAppSettings;
begin
...
function TFMXPythiaModelsSelector.ModelInitialize: Boolean;
begin
...
function TFMXPythiaModelsSelector.ModelListFileCheck: Boolean;
begin
...
function TFMXPythiaModelsSelector.ModelSetDefaultContentIntoFile: Boolean;
begin
  TFileIOHelper.SaveToFile(GetModelListFileName, JSON_MODELS_DEFAULT);
  Result := True;
end;

function TFMXPythiaModelsSelector.ModelsSelectorCategoryAdd: Boolean;
begin
...
function TFMXPythiaModelsSelector.ModelsSelectorCategoryVisible(
...
function TFMXPythiaModelsSelector.ModelsSelectorGetReplaceVersion: Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TFMXPythiaModelsSelector.ModelsSelectorHide: Boolean;
begin
  Result := ExecuteScript(MODEL_SELECTOR_PANEL_HIDE);
end;

function TFMXPythiaModelsSelector.ModelsSelectorSetModelList: Boolean;
begin
...
function TFMXPythiaModelsSelector.ModelsSelectorShow: Boolean;
begin
...
procedure TFMXPythiaCardSelector.CardsContentCreateDefaultFiles;
begin
...
function TFMXPythiaCardSelector.CardSelectorHide: Boolean;
begin
  Result := PostWebMessageAsJson(CARD_SELECTION_DIALOG_HIDE);
end;

function TFMXPythiaCardSelector.CardSelectorSetData(const JsonString: string): Boolean;
begin
  Result := PostWebMessageAsJson(JsonString, 'card-selection-dialog-set-data');
end;

function TFMXPythiaCardSelector.CardSelectorShow(const Dialog: string): Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TFMXPythiaCardSelector.CardSettingsButtonVisible(
...
constructor TFMXPythiaCardSelector.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  CardsContentCreateDefaultFiles;
end;

function TFMXPythiaCardSelector.FilenameRetrieve(
...
procedure TFMXPythiaCardSelector.JSONCardContentDefaultCreate(
  const AType: string);
var
  LType: TChatManagedItemKind;
  JsonAsString: string;
...
function TFMXPythiaCardSelector.TryGetCardFileContent(const AType: string;
...
function TFMXPythiaInputBubble.BubbleInputAudioButtonVisible(
  const Value: Boolean): Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TFMXPythiaInputBubble.BubbleInputClear: Boolean;
begin
  Result := ExecuteScript(INPUT_BUBBLE_RESET_TEMPLATE);
end;

function TFMXPythiaInputBubble.BubbleInputFunctionButtonVisible(
  const Value: Boolean): Boolean;
begin
  Result := PostWebMessageAsJson(
...
function TFMXPythiaInputBubble.BubbleInputMenuClose: Boolean;
begin
  Result := ExecuteScript(CLOSE_INPUT_MAIN_MENU_TEMPLATE);
end;

function TFMXPythiaInputBubble.BubbleInputMenuOpen: Boolean;
begin
  Result := ExecuteScript(OPEN_INPUT_MAIN_MENU_TEMPLATE);
end;

function TFMXPythiaInputBubble.BubbleInputPartialReset: Boolean;
begin
  Result := ExecuteScript(INPUT_PARTIAL_RESET);
end;

function TFMXPythiaInputBubble.BubbleInputSetText(
...
constructor TFMXPythiaInputBubble.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FEnabledButtons := [ebSettings];
end;

function TFMXPythiaInputBubble.GetEnabledButtons: TEnabledButtons;
begin
  Result := FEnabledButtons;
end;

procedure TFMXPythiaInputBubble.SetEnabledButtons(
...
function TFMXPythiaReasoningComponent.ReasoningCollapse: Boolean;
begin
  Result := ExecuteScript(COLLAPSE_REASONING_TEMPLATE);
end;

function TFMXPythiaReasoningComponent.ReasoningExpand: Boolean;
begin
  Result := ExecuteScript(EXPAND_REASONING_TEMPLATE);
end;

function TFMXPythiaReasoningComponent.ReasoningHide: Boolean;
begin
...
function TFMXPythiaReasoningComponent.ReasoningShow: Boolean;
begin
...
function TFMXPythiaReasoningComponent.ReasoningToggle: Boolean;
begin
...
procedure TFMXPythiaAPIKeyManager.ApiKeyValuesUpdate(const KeyName: string);
begin
  if Assigned(FOnApiKeyChanged) then
    FOnApiKeyChanged(KeyName);
end;

constructor TFMXPythiaAPIKeyManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
function TFMXPythiaAPIKeyManager.GetApiKeyNamesAsJsonString: string;
begin
  Result := FApiKeyNamesAsJsonString;
end;

function TFMXPythiaAPIKeyManager.GetApiKeySecretStore: ISecretStore;
begin
  Result := FApiKeySecretStore;
end;

procedure TFMXPythiaAPIKeyManager.SetApiKeyNamesAsJsonString(
  const Value: string);
begin
...
procedure TFMXPythiaAPIKeyManager.SetApiKeySecretStore(
...
{ TFMXPythiaKnowledgeIndexingManager }
...
function TFMXPythiaKnowledgeIndexingManager.RecomputeSendButtonAvailability: Boolean;
var
  Pending: Integer;
begin
  Pending := 0;
...
function TFMXPythiaInputValue.BrowserInput(const AMessage, AKey, AValue,
...
function TFMXPythiaInputValue.BrowserInput(const AMessage, AKey,
...
function TFMXPythiaInputValue.BrowserInput(const AMessage, AKey: string;
...
procedure TFMXPythiaCommandLine.CommandLineInitialize;
begin
  FCommandLine.RegisterPlugin(TApiKeyPlugin.Create(FApiKeyService));
  {--- Custom command are initialized with the FCustomPluginInitializeProc}
end;

constructor TFMXPythiaCommandLine.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TFMXPythiaCommandLine.DispatchCommand(
...
function TFMXPythiaCommandLine.GetApiKeyService: IApiKeyService;
begin
  Result := FApiKeyService;
end;

function TFMXPythiaCommandLine.GetCommandLine: ICommandRegistry;
begin
  Result := FCommandLine;
end;

procedure TFMXPythiaCommandLine.SetApiKeyService(const Value: IApiKeyService);
begin
  FApiKeyService := Value;
end;

procedure TFMXPythiaCommandLine.SetCommandLine(const Value: ICommandRegistry);
begin
  FCommandLine := Value;
end;

function TFMXPythiaCommandLine.TryHandleAsCommand(
...
function TFMXPythiaPath.GetAgentCardsFileName: string;
begin
  Result := GetSupportRawName + '-agent-cards.json';
end;

function TFMXPythiaPath.GetAPIKeyNamesFileName: string;
begin
  Result := GetRawName + '-api-key-names.json';
end;

function TFMXPythiaPath.GetAppJsonSupportFolder: string;
begin
...
function TFMXPythiaPath.GetAppRawName: string;
begin
  Result := ChangeFileExt(ExtractFileName(ParamStr(0)),'');
end;

function TFMXPythiaPath.GetAppSubFolder: string;
begin
  Result := System.IOUtils.TPath.Combine(ExtractFilePath(ParamStr(0)), GetAppRawName);
end;

function TFMXPythiaPath.GetAssetsFolder: string;
begin
...
function TFMXPythiaPath.GetCapabilitiesFileName: string;
begin
  Result := GetSupportRawName + '-capabilities.json';
end;

function TFMXPythiaPath.GetChatSessionsFileName: string;
begin
  Result := GetRawName + '-chat-sessions.json';
end;

function TFMXPythiaPath.GetCustomCardsFileName: string;
begin
  Result := GetSupportRawName + '-custom-cards.json';
end;

function TFMXPythiaPath.GetCustomJSFileName: string;
begin
  Result := GetSupportRawName + '-custom-template-js.json';
end;

function TFMXPythiaPath.GetExchangeDebugFileName: string;
begin
  Result := GetRawName + '-exchange-debug.json';
end;

function TFMXPythiaPath.GetFunctionCardsFileName: string;
begin
  Result := GetSupportRawName + '-function-cards.json';
end;

function TFMXPythiaPath.GetLanguageFolder: string;
begin
  Result := System.IOUtils.TPath.Combine(GetAssetsFolder, LANGUAGE_FOLDER);
end;

function TFMXPythiaPath.GetMcpCardsFileName: string;
begin
  Result := GetSupportRawName + '-mcp-cards.json';
end;

function TFMXPythiaPath.GetMediaFolder: string;
begin
  Result := System.IOUtils.TPath.Combine(GetAssetsFolder, MEDIA_FOLDER);
end;

function TFMXPythiaPath.GetModelCategoriesFileName: string;
begin
  Result := GetRawName + '-model-get-replace-version.json';
end;

function TFMXPythiaPath.GetModelListFileName: string;
begin
  Result := GetSupportRawName + '-model-list.json';
end;

function TFMXPythiaPath.GetParamsConfigFileName: string;
begin
...
function TFMXPythiaPath.GetSkillCardsFileName: string;
begin
  Result := GetSupportRawName + '-skill-cards.json';
end;

function TFMXPythiaPath.GetSupportRawName: string;
begin
  Result := System.IOUtils.TPath.Combine(GetAppJsonSupportFolder, GetAppRawName);
end;

function TFMXPythiaPath.ParamsMainValuesFileNameExists: Boolean;
begin
...
function TFMXPythiaCustomPanels.GetCustomPanels: TCustomPanels;
begin
  Result := FCustomPanels;
end;

procedure TFMXPythiaCustomPanels.SetCustomPanels(const Value: TCustomPanels);
begin
  FCustomPanels := Value;
end;

{ TFMXPythiaCustomJSTemplate }

constructor TFMXPythiaCustomJSTemplate.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FCustomJSTemplate := [];
...
procedure TFMXPythiaCustomJSTemplate.TryToLoadCustomTemplates;
begin
  ...
  if not Reader.IsValid or not Reader.IsArrayNode('template_filename') then
    raise EFMXPythiaException.CreateFmt('%s: invalid JSON content', [Filename]);

  FCustomJSTemplate := Reader.ArrayStrings('template_filename');
  for var Item in FCustomJSTemplate do
    begin
      if not FileExists(Item) then
        raise EFMXPythiaException.CreateFmt('%s: file not found', [Item]);
    end;
end;

{ TFMXAlphaBlend }

class procedure TFMXAlphaBlend.HideWindow(const Form: TForm);
begin
  SetWindowAlpha(Form, 0, True);
end;

class procedure TFMXAlphaBlend.SetWindowAlpha(
  const Form: TForm;
  AAlpha: Byte;
  AEnableLayered: Boolean);
begin
  var H := FormToHWND(Form);
  if H = 0 then
    Exit;

  if AEnableLayered then
    begin
      var Style := GetWindowLongPtr(H, GWL_EXSTYLE);
      SetWindowLongPtr(H, GWL_EXSTYLE, Style or WS_EX_LAYERED);
    end;

  SetLayeredWindowAttributes(H, 0, AAlpha, LWA_ALPHA);
end;
