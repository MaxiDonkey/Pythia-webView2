...
type
  ...
  TToolContainer = record
    ...
    class function ActivateCustomEvent(const ARawJson: string): Boolean; static;
  end;
...
function TVCLChatManagedItemDialogService.DoActivateCustomEvent(
  const ARawJson: string): Boolean;
begin
  Result := TToolContainer.ActivateCustomEvent(ARawJson);
end;
...
begin
  ...
  ShowMessage(
    'Code copy interception' + sLineBreak +
    'Lang = ' + ALang 
  );
end;
...
class function TToolContainer.ActivateCustomEvent(
  const ARawJson: string): Boolean;
begin
  Result := True;
  ShowMessage(ARawJson);
end;
...
begin
  Result := True;
  ShowMessage('Returns the JSON related to the state of the data collector');
  VendorTest.Validation(AState, AOnFinalize);
end;
