...
(*
  ...
  Turn finalization for the pythia-anthropic VCL demo.
  ...
  This unit is shared by both vendor execution paths:
    - the Messages API streaming chat (Demo.Anthropic.Services), and
    - the Managed Agents session flow (Demo.Anthropic.Session.Flow).

  The Messages path uses all three constructors; the Managed Agents path
  uses FromState and FromException (it has no SDK TEventData terminal
  event of its own).
